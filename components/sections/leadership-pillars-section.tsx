          ...
          {LEADERSHIP_PILLARS.map((pillar) => {
            return (
              <ScrollAnimation
                key={pillar.name}
                className={cn(
                  "group relative flex min-h-[320px] w-[min(82vw,22rem)] min-w-[16rem] shrink-0 snap-start flex-col justify-between overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-[0_16px_34px_rgba(12,35,54,0.08)] transition-[border-color,box-shadow,background-color] duration-300 sm:w-[360px] lg:w-auto",
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <div>
                    <p className="font-body text-xs font-semibold tracking-[0.18em] text-muted-foreground uppercase">
                      {pillar.colorLabel} - {pillar.element}
                    </p>
                    <h3 className="mt-2 font-display text-[30px] leading-[34px] font-semibold text-foreground">{pillar.name}</h3>
                    <p className="mt-2 font-serif text-base leading-7 text-muted-foreground">
                      {pillar.meaning} - {pillar.role}
                    </p>
                  </div>
                </div>

                <div className="relative z-10 mt-8 flex items-center justify-between border-t border-border pt-4">
                  <span className="font-body text-sm font-semibold text-foreground">Explore Pillar</span>
                  <Link
