        ...
        <Container className="space-y-10">
          <div className="max-w-3xl space-y-4">
            ...
            <h1 className="font-display text-4xl leading-tight font-bold text-[var(--hero-black)] sm:text-5xl">
              Leadership, Healing, and Wellbeing Pathways
            </h1>
            <p className="max-w-2xl font-serif text-lg leading-8 text-[var(--hero-text)]">
              Explore programmes grounded in kaupapa Maori values that support identity, emotional wellbeing, and practical growth.
            </p>
          </div>
            ...
            {highlightServices.map((service) => (
              <article key={service.title} className="flex h-full flex-col">
                <header className="min-h-[112px]">
                  <h2 className="font-display text-xl font-semibold text-[var(--hero-black)]">{service.title}</h2>
                </header>
                <main className="mt-4">
                  <div className="overflow-hidden rounded-2xl border border-border">
                    <Skeleton className="h-56 w-full rounded-none" />
                    <span className="sr-only">{service.placeholderLabel}</span>
                  </div>
                </main>
                <footer className="mt-4">
                  <p className="font-serif text-sm leading-6 text-[var(--hero-text)]">{service.description[0]}</p>
                </footer>
              </article>
            ))}
          </div>
            ...
            {filteredServices.map((service, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-display text-2xl leading-tight font-semibold text-[var(--hero-black)]">{service.title}</h3>
                  <p className="mt-4 font-serif text-base leading-7 text-[var(--hero-text)]">{service.description[0]}</p>
                  <Link
                    href="/contact"
