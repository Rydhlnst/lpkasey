  ...
  return (
    <>
        ...
        <Container className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {data.members.map((member) => (
              <article key={member} className="rounded-2xl border border-border bg-white p-5 text-center shadow-[0_12px_26px_rgba(15,23,42,0.08)]">
                <PillarColorImage tone={data.tone} className="mx-auto h-14 w-14" />
                <h3 className="mt-3 font-display text-lg font-bold text-[var(--hero-black)]">{member}</h3>
                <p className="mt-1 font-serif text-sm font-bold text-[var(--hero-main)]">{data.subtitle}</p>
              </article>
            ))}
          </div>
        </Container>
      ...
      <section className="bg-background py-14 sm:py-20">
        <Container>
          <div className="grid items-center gap-8 rounded-3xl border border-border bg-card p-6 sm:p-8 lg:grid-cols-2">
            <div>
              <SectionHeader badge="Team Insight" title="Collective Progress Starts with Support" description="Shared values and trusted relationships help each member move from challenge to stability." />
              <blockquote className="mt-4 font-display text-2xl font-bold leading-tight tracking-tight text-[var(--hero-black)] sm:text-4xl">
                &quot;When people feel seen and supported, collective progress becomes possible.&quot;
              </blockquote>
            </div>
